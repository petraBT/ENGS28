...
- The template is given whole except five `#define`s and one expression
...
**No ordering change from her deck, except that her slide 14 (Table 12) is Part 2's datasheet moment, ahead of the prescaler search of slides 11–12.** The other structural difference is that
...
Total: 3+2+6+8+18+10+51+7+5 = **110**.

**Gate 2 rebudget (2026-09-03, `reviews/day15-gate2.md`):** Part 3's third
commit lost its "open `tim.c`" task (Part 3 = 2+5+7+4 = 18) and Part 4 gained
it as its own beat, `act-day15-timc-check`, before the map (Part 4 =
3+1+2+2+4 = 12); Part 5's beats are 5+8+4+5+18+5 with the checkpoint inside the
18-minute build; the Part titles above are the book's; the template has five
`#define` blanks plus the map line; the crucial build carries a student-visible
symptom list (no numbers) ahead of the instructor ladder; and the stretch's
second task is now "one timer count per reading" rather than the prescaler-6
row, which Part 3's reveal already gives.

**Petra's pass 1 (2026-09-04):** the gears commit is gone ("everyone knows what gears do"); her power-conservation note is in the reading and Part 1 is 6 (2+4). The `tim.c` cross-check activity is gone with the code made consistent (Part 4 = 10, 3+1+2+4). The push commit is hypothetical, never done on the servo. The map's round-number mechanic is gone ("don't give it away"). Part 5 takes the minutes: 51 (5+8+4+5+24+5).
