...
| 13/8 | Breakout photo + STEMMA wiring (2 images, color callouts) | **Rebuild with annotations** — this is the week's wiring figure.  **UPDATE 2026-08-30: Petra dropped `stemma.png`, her own annotated cable photo (wire labels on the leads) — now the base64 raster inside `stemma_wiring.svg`'s right panel (`fig-accel-wiring`).  Keep `stemma.png` committed for reproducibility; the color-callout list was removed as redundant with her annotations.  Pass 2 (2026-08-30): the photo's own `5V` wire label is edited to `3.3V` in the embedded raster, at her request — matched to the GND label's face and size; her original `stemma.png` is untouched** |
| 13/9 | Full test-setup "photo" | **CORRECTED on inspection (session 2): not a photo — a stale Fritzing diagram whose sensor is an MPU-6050, red lead on a 5 V pin.  NOT USED.**  **UPDATE 2026-08-30: Petra dropped `Accel_bb.png`, a whole-bus Fritzing (display + accelerometer on shared rows) — now `fig-accel-bb`.  Pass 2 (2026-08-30): projected WHOLE — her ruling; the earlier `accel_bb_bus.png` top-crop cut the Nucleo off and is deleted.  **Same day she delivered `accelerometer_noDisplay.png`, the display-free re-export — now the `fig-accel-bb` image; `Accel_bb.png` stays committed as the display-included record.**  Measured at Gate 3′: the drawn red lead is on **+3V3** (the 5V socket is drawn empty; the +3V3 label hides under the wire) — the drawing is correct.  The breakout drawn is Fritzing's MPU-6050 part standing in (her library has no LSM303AGR part); she struck the caption sentence disclosing it (pass 2)** |
| 13/14–15/25 | AD2 digital-channel wiring (4 images, "Connect me!" callout) | **CORRECTED on inspection (session 2): two of the four images are unusable (an ADALM2000 pinout — wrong instrument; an Arduino UNO + MPU-6050 Fritzing).  The clean AD2-with-bundle product photo (slide14_img4) IS used, as `ad2_digital_bundle.png` in `fig-ad2-digital-wiring`; the Waveforms steps are Day 9x's `fig-waveforms-decode` by refPage — Gate 3′ later made the setup slide text-only (the Day 9x screenshot has 20 µs/div baked into its pixels; Day 13 teaches 50)**.  **UPDATE 2026-08-30: Petra dropped `accelerometerSlide.png` — her slide 25 at full export resolution (2468×1854).  Its pinout sheet, ~2× the old crop, is `ad2_connector_labels_hires.png`, the right panel of `fig-ad2-digital-wiring` (the old `ad2_connector_labels.png` stays on disk, unreferenced).  Pass 2 (2026-08-30): the strip is the WHOLE sheet, all fifteen columns — her ruling ("show the whole thing") over the Gate 3′ digital-half legibility crop; the ~1.7 % digits are accepted.  The sheet itself is the ADALM2000's (AD2 silkscreen reads T1/T2, sheet reads TI/TO) — escalated to Petra as a "leave it and say so"; ask for the AD2's own sheet for next year, not this delivery** |
| 13/19–21 | Waveforms captures A/B/C | Her originals cropped to the trace rows (`waveforms_capture_a/b/c.png`).  **A is re-cut tight to its one transaction and reads well projected.  B and C fill their frames, and Petra exports one way (2026-08-27) — so the gate's named fallback shipped: write-half/read-half close-ups (`_write`/`_read` crops) carry the readable bytes, while the whole strips stay as the discovery and the rescue's marking target.**  Note her captures are at **50 µs/div**, not Day 9x's 20 — Gate 2′ made this a taught change |
...
| 13x/3 | Mass-spring-case diagram | Rebuild with labels (rough chapter uses plain pull).  **BUILT 2026-08-30 (session 3): hand-authored `mass_spring.svg` (her drawing redrawn — m, k, x, 0 rest, accel, F = kx = ma) plus `mass_spring_rest.svg`, the at-rest variant the commit slide projects (P-15: the displaced drawing is the reveal's)** |
| 13x/4 | Capacitive displacement diagram | Rebuild.  **BUILT 2026-08-30: hand-authored `capacitive_pickoff.svg` (her slide-4 idea; her composite's two connector arrows lose their anchors when cropped)** |
| 13x/5 | MEMS micrograph + mechanism (2 images) | Rebuild; watch the multi-picture composite trap.  **USED 2026-08-30 as two panels — but the mechanism's right (displaced) panel draws the plate moving WITH its acceleration arrow, the wrong answer to the mass commit: the book shows the at-rest panel only (`mems_mechanism_rest.png`) until Petra rules on the panel (ask A2); her Roger Howe credit restored to the caption** |
| 13x/7 | LSM303AGR block diagram | Rebuild (callout "we only use the accelerometer").  **BUILT 2026-08-30: `lsm303agr_block.svg` (her blue accel box + red SCL/SDA box) for the book; `lsm303agr_block_accel.svg`, the accelerometer-half crop, for the slide (full-diagram labels measured 1.1–1.5 % of slide height — below the floor).  Gate 2': SIX sense capacitors (a pair per axis) into a MUX → one shared amplifier → one ADC — the rough draft's "three, one per axis" was wrong** |
| 13x/8 | Datasheet spec tables (3 images, callouts: full range / 2^bits, 12/10/8-bit) | Rebuild — this is the Day 13x datasheet moment's figure.  **BUILT 2026-08-30: her annotated sensitivity excerpt as `accel_specs_sensitivity.svg` (reveal) and `accel_specs_masked.svg` (commit — answer column blanked, purple answer boxes stripped, her mode brackets kept); `accel_specs_header.png` (Table 3 title + Min/Typ/Max header row) restores the column headers on the commit slide; `accel_specs_range.png` + `accel_specs_offset.png` (re-cropped to the row boundary) complete the book figure.  Gate 2': the offset is TYPICALLY ±40 mg, Min/Max ±80 — "up to ±40" was wrong** |
| 13x/9 | Breakout schematic with 4 callouts (pullups, header, regulator, I2C strapping) | Rebuild.  **BUILT 2026-08-30 as `breakout_schematic.svg` — but Gate 2' found the composite defective: the clip discards the source's STEMMA/I2C Headers block (right 29 %, bottom 18 %) and every callout is displaced up-left of its target.  NOT patched (the Day 9 lesson); the false caption sentence is fixed and the original export is on the ask-Petra list (A1).  Ships interim for its 1-minute do-not-linger beat** |
| 13x/17 | ST read-transfer diagram + auto-increment quote | Rebuild (same base as 13/29's read half).  **RESOLVED 2026-08-30 by reuse: the slide projects Day 13's `i2c_transfer_reads.svg` whole (Tables 22–23 with her gloss row and her auto-increment line already in it).  SHARED with Day 13's `sl-day13-transfer-reads` — never re-crop it in place** |
