...
| 13/9 | Full test-setup "photo" | **CORRECTED on inspection (session 2): not a photo — a stale Fritzing diagram whose sensor is an MPU-6050, red lead on a 5 V pin.  NOT USED.**  **UPDATE 2026-08-30: Petra dropped `Accel_bb.png`, a whole-bus Fritzing (display + accelerometer on shared rows) — now `fig-accel-bb`.  Pass 2 (2026-08-30): projected WHOLE — her ruling; the earlier `accel_bb_bus.png` top-crop cut the Nucleo off and is deleted.  She would rather the display not appear at all: a display-free version needs her Fritzing re-export (asked); do not pixel-erase it.  Measured at Gate 3′: the drawn red lead is on **+3V3** (the 5V socket is drawn empty; the +3V3 label hides under the wire) — the drawing is correct.  The breakout drawn is Fritzing's MPU-6050 part standing in (her library has no LSM303AGR part); she struck the caption sentence disclosing it (pass 2)** |
