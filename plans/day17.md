...
GAP, advertising and connecting (slide 9); GATT, services and
characteristics (slide 10, written GATT); the message-board takeaway
(slide 11); the Bluefruit as a UART bridge with its two modes (slide 12);
the module tour against the Adafruit guide the lab links (slide 17's
photo: MOD, CTS, TXO, RXI, VIN, GND, RTS, DFU, the UART/CMD switch, VIN
at 3.3 V).  Reading questions on these ideas only.  No register content,
no pin choices, no code.
...
**Parts** (announcements + settle 4, close 1; beats sum to 110):

| Part | min | Mode | What happens |
| --- | --- | --- | --- |
| — | 4 | — | title, groupings (hers), where we are, agenda |
| 1. The Bluefruit needs USART1 | 8 | explain | her slides 12–13: the module bridges BLE to a USART; it requires USART1 and hardware flow control; the RM and the AF tables answer how, and the pin choice is Deliverable 1.  Datasheet moment.  (2 + 4 + 2) |
| 2. Your USART1 driver, and the passthrough test | 14 | explain | her slides 14–16: mimic the USART2 driver; the non-blocking RxAvail rule in her words; `passthrough.c` projected whole with `uart2_RxAvail()`; the AD2 Protocol-tab test, one character at a time.  (4 + 6 + 4) |
| 3. Connecting the Bluefruit, and its two modes | 16 | explain | her slides 17–22: the pin-to-signal table (the lab's crossover), 3.3 V, the app; UART mode test; CMD mode and the three AT commands; the MOD pin and the startup sequence (Deliverable 5's brief); "you are now ready to build your system."  (4 + 3 + 4 + 3 + 2) |
| 4. The final project and your paper design | 7 | explain | her slides 23–24: read the handout, the paper design in her words, due Friday 10 pm; kit-return line (pending Q8).  (5 + 2) |
| 5. Project work time | 60 | do | "We'll now give you some time to work on your project." — one sentence; the room works on Lab 9.  Leave the paper-design slide up. |
...
| 24 paper design | Part 4 (her words; due Friday) |
...
**Hand-offs.**  In: Day 16's close named this reading.  Out: Lab 9 §3's
paper design due Friday 10 pm (hers); demos Thursday.  The Reference
section collects the flow-control mechanism (RM0490 §24.5.20, CTSE/RTSE
in USART_CR3), the AT-command startup timing rules from Lab 9's
"Important!" boxes, and the AF option lists — without resolving
Deliverable 1's choice.
