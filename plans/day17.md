...
| 1. Using USART1 and hardware flow control (her slide 13's title) | 8 | explain | her slides 12–13: the module bridges BLE to a USART; it requires USART1 and hardware flow control — glossed in plain words first (CTS and RTS are two extra wires the two sides use to say wait / go ahead, so neither sends faster than the other can take in), then RM0490 §24.5.20's fact stated plainly: with CTSE enabled, the transmitter checks CTS before every frame, and a CTS left unwired (the module pulls its side high) means transmit silently does not occur.  The RM and the AF tables answer the how, and the pin choice is Deliverable 1.  Datasheet moment.  (2 + 4 + 2) |
