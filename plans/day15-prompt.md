...
1. **Re-check the ground truth against what Petra has answered.** As of
   ...
   the center / signal). No homework is due Thursday. The two-channel ADC read and the `adc.c`/`adc.h` library are Lab 5's.
   Still open: **1b** (the current figure) and **6**'s endpoints (what
   students see at 1 ms and 2 ms). Where
   an answer is
