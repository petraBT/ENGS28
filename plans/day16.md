...
> two channels print on their screen, the servo was confirmed following the
> pot before the pot came out, and the student has written down the loop's
> update rule with its sign, its bounds and its two tuning numbers.**
> *(Calibrated to her deck, which ends on "discuss how you might implement
> the feedback loop" and whose last figure has no potentiometer — question 7.)*
...
- The two-channel program is Wednesday's, with the pot's channel now a
  photocell's (or, if Wednesday's decision point moved the do-step here, it
  is written now, funded from Part 3).
...
- **Checkpoint at minute 38** (Part 2, after the build): a node that reads a
  ...
  the same value is the sequencer (Wednesday's reveal, said again once).
  **Part 4's ladder** (minute 78): no motion → power and ground, Wednesday's
  ladder; motion that ignores the knob → the servo program is not the one on
  the board; off-center → the knob was re-seated on Tuesday, turn it.
...
asks for. (This is Lab 8 D9's work started early, which is the lab's intent
for the afternoon; nothing about it appears in the book beyond the design.)
...
| 10 (servo test setup review) | Part 4's first beat (Wednesday's build, still wired; Tuesday's program re-flashed) |
| 11–13 (assembly: cups, clips, recreate the circuit; **remove the pot**) | Part 4 — the pot comes out once the servo is confirmed, her sentence |
| 14 (ultimate setup: two dividers and the servo, **no pot**) | Part 4's end-state figure — her export of this slide is asked for (follow-up 3b: the extracted version lost the regulator board) |
...
| 2 | 33 | do | **Lab 8 Part 1, in class.** The lab's §2 as printed; the book describes the activity and never its answers. Datasheet lookups (D1) and multimeter measurement (D2–3), at the table (presenter note: a closed fist over the cell is dark enough) (6). R2 by the geometric-mean rule (D4) and the expected voltages (D5–6), on paper — the *why* of the rule taught in one sentence: the mid-value puts the dark and light readings farthest apart on the ADC's scale (7). Wire two dividers with the kit's 10 kΩ (Lab 8 Figure 2), AD2 voltmeter on both nodes (8). **Checkpoint minute 38**: a node at a rail value is a divider on the wrong row (1). Wednesday's two-channel program with the two photocell nodes; one-second period; D7–D8 with the flashlight (9). Checkpoint minute 50 (2) |
...
| 4 | 17 | do | **On to the arm.** Tuesday's servo program back on the board: the servo still follows the pot — confirm it (her slide 10); the ladder above if it does not (5). Then her sentence: once it is confirmed, the pot comes out, and its channel is free (which pins the pot and the photocells use on Thursday: follow-up 3b) (1). Photocells out of the breadboard and into the shielding cups; alligator clips from their leads back to the same breadboard rows, recreating the two dividers (her slides 11–13; Lab 8 App. A) (8). Both channels print again, now from the arm; sweep the flashlight across the arm and write down which channel rises as the light moves which way — the sign of the loop's step (3) |
...
| 6 | 10 | do | **Start the build.** Pseudocode of the loop at the table — read both channels, error, update, bound, write, wait T — kept as words, not as the course's function names, then the fixed-step version if there is time — the stretch (8). What Lab 8 D9 and D10 ask, in one sentence each (2) |
...
8 beside Lab 8 Figure 6 — hand-author) and her exports of the Day 16 wiring
slides with the regulator board in them (follow-up 3b).
