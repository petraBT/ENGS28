...
| **L-11** | **A day says WHEN, never WHAT.** Where a day is written at all it is "on Day N", never "in Day N" (or "tomorrow", since Day Nx follows Day N). But in **student-facing text a course-internal day must not be used as the name of a topic** at all: say what the thing was, not when it happened. Petra's Day 9 hand pass (`0b0bba9`) struck every one of them — ~~"On Day 8"~~ → "Last week", ~~"the Day 3 idiom"~~ → "the same one we used before" — and she confirmed the ruling corpus-wide (2026-09-17): *"I don't like the day references but since they kept coming up I gave up on that as a more minor issue. If it's a relatively easy sweep I'd much prefer referencing topics rather than days."* The reason is B-11b: a reader who missed that day, or who lands on a subsection from search, has no course calendar to reconstruct — ~~"the same timer we set up for periodic interrupts on Day 8"~~ loses nothing by dropping the clause, because the sentence already names the topic, and most of them are like that. **Four things keep their day:** a section or chapter `<title>` that the day names ("Day 9 In-Class: GPIO Interrupts"), which is structural (B-1, S-20); `<instructor>` blocks, `<note>` and a deck's `presenterNote`, which are presenter-facing, the same carve-out L-18 makes for Part numbers; a day as a plain adverbial of time ("we'll see on Thursday why", "tomorrow"), which is hers and is correct; and a possessive naming real coursework ("Tuesday's timer program is what is due"). A `<title>` that merely *mentions* a day is not exempt — ch-motors' table title "The five writes, on Day 9's line and on Day 12's" reads better, and more precisely, as "on PB4's line and on PA15's". `check_rules.py` **errors** on the student-facing form, with those exemptions scoped. The sweep landed on 2026-09-17: all 66 occurrences were cleared across nine chapters and `assets/decks/day13.json`, and the severity was promoted from warn to error in the same commit, so a new one fails the check rather than scrolling past. |
...
| **L-19** | Every reference-manual or datasheet mention in book prose links to its hosted PDF at least once per subsection (or per enclosing `<section>` when no `<subsection>` wraps it) — from the whole-book style sweep's contract, `plans/style-sweep.md` C-1/C-2/C-3. A mention whose only occurrence sits inside a projected block (a `<slide>`, or any `<activity>`/`<task>`/`<instructor>`/`<table>`/`<figure>` a deck refs) is exempt: nothing projected may carry a link (L-21), and if no non-projected paragraph in that scope names the same document, the mention stays unlinked by necessity rather than by oversight — that gap is reported, not silently fixed, since fixing it means writing new prose. |
| **L-20** | Malformed display form for the style sweep's link contract: a `§` written as the `&#167;` entity instead of the literal character; a number and its unit with no space between them ("5V", not "5 V"); a reference-manual `<url>` whose visible text is anything but "RM0490" or "reference manual" (the § or Table/Figure number stays outside the link, as plain text); a datasheet `<url>` whose visible text does not read as "*Part* datasheet" (AN-1057 is the one designator kept bare, since an application note is genuinely known by its number); a `#page=N` anchor on any hosted PDF link (never supported, `plans/style-sweep.md` C-4); or a typed "section N.N" that means the reference manual or a datasheet with no designator anywhere in the same sentence — the L-14 trap, extended into a lintable form. |
| **L-21** | Nothing projected may carry a link — the sweep's extension of the standing no-links-in-slides rule. A `<url>` inside a `<slide>` block, or inside any `<activity>`/`<task>`/`<instructor>`/`<table>`/`<figure>` a deck JSON refs (built from every `"slide"` value across `assets/decks/*.json`, since a deck can ref either a `<slide>` element or a wholesale-projected activity/instructor block directly) is always an error, whichever document it points to. A bare `<xref>` is different: the player's `dexref()` neutralizes one inside a projected activity/task/instructor/table/figure at render time, so `check_rules.py` only warns on an `<xref>` sitting directly inside a `<slide>` block, where nothing neutralizes it. |

| **L-22** | There are no lab benches. Every student carries a portable kit and works on their own laptop, so "on the bench", "at your bench" and "lab station" all describe a room that does not exist in this course. Lintable, and lint it: the word survived a committee gate in `ch-motors.ptx` and then went into new Day 7 prose, because nothing was checking. |
`scripts/check_rules.py` enforces L-1 … L-11 and L-22, and warns on L-12, plus B-9, B-11,
B-11a, B-14, B-15 and S-4, image paths, unresolved cross-references and step
counts. Run it before every committee review.

It also enforces the style sweep's link contract: L-21 (a link anywhere
projected) is an error; L-19 (an unlinked mention) and L-20 (a malformed
display form) warn, since both need a human to judge whether the mention is
specific enough to link or generic enough to stay plain.  L-11 is split the same
way: "in Day N" is an error, while a day used as a topic name in student-facing
text warns, because each one needs a topic chosen for it and the corpus-wide
sweep is authorized but not yet done. L-21's projected-id
set comes from `assets/decks/*.json`, read fresh on every run, so it always
reflects the current decks.
...
```xml
<instructor>
    <p>Here are the three functions, complete:</p>
