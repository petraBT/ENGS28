...
| ch-accelerometers.ptx | Day13, Day13x, Day14 | **Day 13 awaiting Petra's pass 2; Day 13x awaiting her book read; Day 14's book and deck both delivered, her pass-1, pass-2, and pass-3 comments applied (2026-09-02), connecting prose complete — awaiting her next look**: the reading, the in-class Parts 2–10 with full connecting prose (no DELIVERY-2 markers remain), the Reference section (now with a differential-capacitor figure), and the 50-slide `day14.json`, fit-swept at 1600×900; two committee gates plus three comment passes applied and verified item by item; four listings registered in `check_starters.py`; ask-Petra list in `plans/week7-handover.md` (plain-view CoolTerm shot — reminder standing, no accelerometer set up currently, and when it arrives the two control-register readouts need blurring, same reason as the Part 2 reveal; negative-raw-print question and the padding-bits check, both waiting on hardware; Part 7 chart-view minutes in class); Q3 still open |
