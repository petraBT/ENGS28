...
| ch-accelerometers.ptx | Day13, Day13x, Day14 | **Day 13 through Gate 3′, list applied (2026-08-30) — awaiting Petra's pass 2**: her 37 pass-1 annotations applied and archived, her three new images integrated (stemma cable photo, whole-bus Fritzing, hi-res AD2 pinout crop), in-class connecting prose written, `reviews/day13-gate3.md` (7 reports + synthesis, 19 items applied; transfer-pattern slide split write/read per the recorded fallback), 46-slide deck fit-swept at 1600×900; Q3 still open (nothing student-facing asserts the wrong-address print; Part 6 debrief ¶ stays owed on it); ask-Petra list refreshed in `plans/week7-handover.md`; next is session 3 (build Day 13x, `plans/day13x-prompt.md`) |
