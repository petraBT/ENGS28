...
| ch-accelerometers.ptx | Day13, Day13x, Day14 | **Day 13 built through Gate 2′, list applied (2026-08-27) — awaiting Petra's pass 1**: chapter skeleton for the whole week (Day 13x/14/Reference are marked placeholders), Day 13 reading + in-class skeleton + 43-slide deck (`assets/decks/day13.json`), `reviews/day13-gate2.md` (11 reports + synthesis, 30 items applied and verified); Q3 still open (nothing student-facing asserts the wrong-address print); ask-Petra list in `plans/week7-handover.md`; next is session 3 (build Day 13x, `plans/day13x-prompt.md`) |
