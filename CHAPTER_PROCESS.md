...
| ch-accelerometers.ptx | Day13, Day13x, Day14 | **Day 13 awaiting Petra's pass 2; Day 13x awaiting her book read; Day 14 through Gate 2′ with the list applied (2026-08-31) — awaiting her pass 1**: Day 14 reading (the guided datasheet walk + stationary-reading subsection, Gate 1.5 applied), in-class skeleton Parts 2–10 (activities with instructor reveals, DELIVERY-2 markers naming owed prose), the chapter's Reference section (all seven manifest items, B-10), and the 51-slide `day14.json` fit-swept at 1600×900; `reviews/day14-gate2.md` (12 reports + synthesis, 22 items applied and verified item by item — the stationary-rule sign, the upright-frame precondition for atan2, the impossible signed-shift example, the honest BRR recall with the cast mechanic, (int)round(), the ladder rewritten off the Q3 precedent); four listings registered in check_starters.py; ask-Petra list in `plans/week7-handover.md` (partial.c Table-13 comment, plain-view CoolTerm shot, exclude screenshots, Reference-opener ruling, padding-bits check, Part 7 chart-view minutes); Q3 still open; next is her pass on Day 14, then the in-class connecting prose from the slides she passes |
