...
| ch-photosensors.ptx | Day16 | **Day 16 book passed by Petra (three passes, 2026-09-08, 61 comments applied); Day 16 deck through Gate 3, awaiting her pass** (`assets/decks/day16.json`, 35 entries, 30 refs, 5 instructor-only; sixteen `<slide>` blocks; `reviews/day16-gate3.md`). Her ruling on the book: the in-class section is her sixteen slides and nothing more, with Lab 8's own deliverables projected while the class works; Parts 1–5 (the divider question and the nonlinearity; Part 1 of Lab 8; assembly; read section 4 and discuss; lab time). Gate 2 in `reviews/day16-gate2.md`; the player gained a `\sqrt` handler and `check_deck.py` a raw-LaTeX check |
| ch-ble.ptx | Day17 | **Day 17 book through Gate 2, awaiting Petra's pass 1** (2026-09-08): reading (BLE ideas, her slides 6–12) with five reading questions, in-class Parts 1–5 (her slides 13–26 plus Lab 9's wording; briefing then 57 min work time), Reference (flow control, the Protocol tab, AT commands); `passthrough.c` from her code slide; thirteen Gate 2 reports and the synthesizer's 27-item list in `reviews/day17-gate2.md`, applied and verified; `inst-day17-af-options` unprojected until delivery 2; questions in `plans/week9-ground-truth.md` §9 and `plans/week9-handover.md` |
| ch-power.ptx | Day17x | **Day 17x book through Gate 2, awaiting Petra's pass 1** (2026-09-08): x-day, no reading; the work session as the hour's frame with the optional sleep-modes unit as Parts 1–4 from her deck (her slide 13 LPMS erratum and the /8 callout corrected against RM0490 and flagged); `watchdog.c` from her code slide; ten Gate 2 reports and the synthesizer's list in `reviews/day17x-gate2.md`, applied and verified; her slides 22–25 deferred pending Q1; three `inst-day17x-*` blocks unprojected until delivery 2 |
