...
| ch-photosensors.ptx | Day16 | **Day 16 book through Gate 2, awaiting Petra's pass 1 (2026-09-07)**: introduction, the reading with five questions, Parts 1–6 (nine activities, four `<instructor>` blocks), Reference; eight figures (the divider redrawn as an SVG, her exports raw, Lab 8 Figure 6 extracted); Gate 0 as verification and the Gate 1 re-check in `plans/week8-ground-truth.md` and `plans/day16.md`; fourteen Gate 2 reports and the synthesizer's list in `reviews/day16-gate2.md`, applied and verified phrase by phrase; `check_deck.py` reports the four `inst-day16-*` blocks as NOT PROJECTED (the delivery-1 state). No deck yet |
