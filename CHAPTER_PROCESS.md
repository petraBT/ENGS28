...
| ch-accelerometers.ptx | Day13, Day13x, Day14 | **Day 13 awaiting Petra's pass 2 (through Gate 3′, 2026-08-30); Day 13x through Gate 3′ with prose, list applied (2026-08-31) — her passes 1 and 2 on the deck applied (53 + 19 annotations); awaiting her read of the book section**: Day 13x skeleton (Parts 1–5 + close, three committed activities with projected reveals, figures rebuilt from her deck with annotations, the masked/reveal sensitivity pair, DELIVERY-2 markers naming what each subsection owes) + 32-slide `day13x.json` fit-swept at 1600×900; `reviews/day13x-gate2.md` (11 reports + synthesis, 24 items applied and verified item by item — the block diagram's six shared-chain capacitors, the ±40-typical/±80-max offset, the high-pass CTRL_REG2, the sign-bit picture, the worked example re-routed off the un-derived 2¹⁶ formula); ask-Petra list refreshed in `plans/week7-handover.md` (breakout-schematic crop A1, MEMS right panel A2); Q3 still open; next is her pass on Day 13x, then session 4 (build Day 14, `plans/day14-prompt.md`) |
